...
public class Mob {
    ...
    public void setDamage() {
        ...
        if (this.template.id == MobName.TU_HA_MA_THAN || this.template.id == MobName.TUONG_GIAC || this.template.id == MobName.MY_HAU_VUONG) {
            this.damageOnPlayer = this.damageOnPlayer / 100 * 5;
        }
        ...
        if (pl != null && !pl.isDead) {
            ...
            try {
                ...
                if (!isMobMe) {
                    ...
                    } else if (zone.tilemap.isNormal()) {
                        dameHp = dameHp * 3; // dame quái thường
                    } else if (zone.tilemap.isDauTruong() || zone.tilemap.isLoiDai()) {
                        dameHp = dameHp * 10 / 100;
                    } else if (zone.tilemap.isLangCo()  || zone.tilemap.isFujukaSanctuary() ) {
                        dameHp = dameHp * 9; // dame quái làng cổ
                    } else if (zone.tilemap.isLangTruyenThuyet() ) {
                        dameHp = dameHp * 4; // dame quái ltt
                    } else if (zone.tilemap.isVDMQ() ) {
                        dameHp = dameHp * 3; // dame quái vdmq
                    }
                }
                dameHp -= pl.dameDown;
    ...
    public void dead(Char killer) {
        if (killer != null) {
            ...
            if (this.template.id == MobName.NGUOI_TUYET) {
                ...
                if (this.isBoss) {
                    if( zone.tilemap.isNormal() || zone.tilemap.isVDMQ() ){
...
//                    }
                    ...
                    if (this.template.level < 80) {
                        killer.addTopThanNong(1);
                    }
                } else {
